...
ssh -i $HOME/.ssh/deploy_rsa $TARGET_USER@$TARGET_HOST 'supervisorctl stop spacemuffin-daemon'
...
ssh -i $HOME/.ssh/deploy_rsa $TARGET_USER@$TARGET_HOST 'supervisorctl start spacemuffin-daemon'
